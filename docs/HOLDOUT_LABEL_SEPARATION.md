...
| Public bench partitions | [BENCHMARK.md](BENCHMARK.md) provenance section; `benchmarks/formal_pr_bench/manifest.v1.json` |

Template-development cases listed in `benchmarks/formal_pr_bench/template_dev_cases.json` must never be counted as FormalPR-Bench `held_out` evaluation. The holdout runner and FormalPR-Bench scorer both fail closed on that contamination.
