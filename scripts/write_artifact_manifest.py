...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--kind", action="append", default=[], help="Optional artifact kind, aligned with --artifact order")
...
def main() -> int:
    ...
    entries = [
        artifact_entry(Path(path), kind=kind, root=args.root)
        for path, kind in zip(args.artifact, kinds, strict=True)
    ]
