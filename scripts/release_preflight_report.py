...
def _check_multi_lane_manifest() -> list[str]:
    ...
    with TemporaryDirectory() as tmp:
        ...
        from ovk.core.multi_lane import manifest_material_paths
                ...
                make_bundle(
                    [
                        evaluate_validated_authorization_path(
                            read_json_file(Path("examples/auth_regression/input_admin_protected.json")),
                            repo="smoke/repo",
                            head_sha="smoke-head",
                        )
                    ]
                ...
                make_bundle(
                    [
                        evaluate_infra_exposure(
                            read_json_file(Path("examples/infrastructure_exposure/input_private_sensitive_resource.json")),
                            repo="smoke/repo",
                            head_sha="smoke-head",
                        )
                    ]
            ...
            (
                ...
                run_sprint1_self_protection(
                    metadata=build_metadata_from_inputs(
                        metadata_path=Path("examples/no_agent_self_approval/input_gate_preserved.json"),
                    ),
                    repo="smoke/repo",
                    head_sha="smoke-head",
                ).bundle,
...
def _check_formal_pr_bench() -> list[str]:
    ...
    scores, _leaderboard = run_benchmark(expanded=False, include_extended=True)
