...
def test_authorization_schema_rejects_missing_routes_fixture() -> None:
    errors = list(
        validator().iter_errors(load_json("examples/auth_regression/input_malformed_missing_routes.json"))
    )
    assert errors
...
def test_authorization_schema_rejects_bad_witness_fixture() -> None:
    errors = list(
        validator().iter_errors(load_json("examples/auth_regression/input_malformed_bad_witness.json"))
    )
    assert errors
