"""WP-17 release-ledger authorization and provenance-boundary tests."""
...
def test_independently_resolved_workflow_provenance_authorizes() -> None:
    ledger = _complete_ledger()
    records = _live_runs()
    ...
    provenance = authorized["evidence"]["workflow_provenance"]
...
def test_verifier_fail_closed_on_missing_workflow() -> None:
    ...
    ok, _, authorized = verify_release_ledger(
        ledger,
        repo_root=REPO,
...
def test_live_head_sha_mismatch_cannot_authorize() -> None:
    ledger = _complete_ledger()
    records = _live_runs()
...
def test_live_workflow_name_mismatch_cannot_authorize() -> None:
    ledger = _complete_ledger()
    records = _live_runs()
    first = next(iter(records))
...
def test_live_workflow_path_mismatch_cannot_authorize() -> None:
    ledger = _complete_ledger()
    records = _live_runs()
    first = next(iter(records))
...
def test_live_repository_mismatch_cannot_authorize() -> None:
    ledger = _complete_ledger()
    records = _live_runs()
    first = next(iter(records))
    records[first] = {
...
def test_live_unsuccessful_run_cannot_authorize() -> None:
    ledger = _complete_ledger()
    records = _live_runs()
    first = next(iter(records))
...
def test_ledger_cannot_self_assert_authorization_or_provenance() -> None:
    ledger = _complete_ledger()
    ledger["release_state"]["authorized"] = True
    ledger["release_state"]["verified_source_sha"] = SHA
    ledger["evidence"]["workflow_provenance"] = {
        "verifier": "forged",
        "candidate_sha": SHA,
    }
    failures = validate_release_ledger_structure(ledger, repo_root=REPO)
