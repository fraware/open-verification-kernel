...
def test_cedar_backend_pass_and_fail() -> None:
    passed = evaluate_cedar_policy(json.loads(Path("examples/backends/cedar_pass.json").read_text(encoding="utf-8")), repo="r", head_sha="sha")
    failed = evaluate_cedar_policy(json.loads(Path("examples/backends/cedar_fail.json").read_text(encoding="utf-8")), repo="r", head_sha="sha")
